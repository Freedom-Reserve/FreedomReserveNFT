...
import ERC20Token from "./ERC20_BSC2.json";
...
//--------------------------==
export const getERC20TokenBalance = async (compo, userAddr) =>
  new Promise(async (resolve, reject) => {
    log1("---------== getERC20TokenBalance()");
    const [instERC20ERC20Token, acct0] = extractCompo(compo, "tokenERC20Token", 0);
    if (instERC20ERC20Token === undefined || acct0 === undefined) {
      resolve(-1);
      return false;
    }
    ...
    try {
      const data = await instERC20ERC20Token.methods.balanceOf(userAddr).call();
      log1("data:", data);
      resolve(data);
    } catch (err) {
      console.error("getERC20TokenBalance() failed.", err);
      reject(-1);
    }
  });
//--------------------------==
